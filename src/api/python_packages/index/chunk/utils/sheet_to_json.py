...
from .convert_ods_to_xlsx import convert_ods_to_xlsx
...
def sheet_to_json(filepath: str, section_name: Optional[str] = None, include_fields: Optional[List[str]] = None, header_row: int = 1) -> List[OrderedDict]:
    ...
    if ext == '.ods':
        ...
        if convert_ods_to_xlsx(filepath, xlsx_path):
            try:
                result = _process_xlsx(xlsx_path, section_name, include_fields, header_row)
            ...
            logger.error(f"Failed to convert ODS to XLSX: {filepath}")
