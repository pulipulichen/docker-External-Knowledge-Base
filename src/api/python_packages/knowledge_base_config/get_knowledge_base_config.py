...
def get_knowledge_base_config(knowledge_id):
    ...
    """
    CONFIG_DIR = os.path.join(os.path.dirname(__file__), '../../../', 'knowledge_base/configs')
    ...
    if not os.path.exists(config_path):
        ...
        if not os.path.exists(config_path):
            logger.error(f"Knowledge base config file '{knowledge_id}.yml' not found at {config_path}", stack_info=True)
            return {}
